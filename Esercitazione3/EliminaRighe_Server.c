/* Server che riceve un file e lo ridirige senza la linea fornita dal client al client */
...
int main(int argc, char **argv)
{
	...
	printf("Server: creata la socket d'ascolto per le richieste di eliminazione righe, fd=%d\n", listen_sd);
	...
	for(;;){ 
		...
		* corrisponde la costante EINTR!*/
		...
		if (fork()==0){ // figlio
			...
			printf("Server (figlio): eseguo l'eliminazione linea\n");
            ...
            char r;
			//Ricezione file
            int i= 1;  //Conteggio linea
            while(read(conn_sd, &r, 1) == 1){
                if(i != index){
                    write(conn_sd, &r, 1);
                }
                if(r == '\n'){
